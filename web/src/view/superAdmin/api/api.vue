<template>
  <div>
    ...
    <el-drawer v-model="syncApiFlag" :size="appStore.drawerSize" :before-close="closeSyncDialog" :show-close="false">
      ...
      <el-table v-loading="syncing || apiCompletionLoading" element-loading-text="小淼正在思考..."
      ...
      <h4>
        已删除路由
        <span class="text-xs text-gray-500 ml-2 font-normal">已经不存在于当前项目的路由中，确定同步后会自动从apis表删除</span>
      </h4>
      ...
      <h4>
        忽略路由
        <span class="text-xs text-gray-500 ml-2 font-normal">忽略路由不参与api同步，常见为不需要进行鉴权行为的路由</span>
      </h4>
